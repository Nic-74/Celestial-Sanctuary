...
The owner approved switching to direct Drive access on September 18, 2026. Google consent requests the broad drive scope. The application verifies that file reads belong to the configured folder and only creates files in that folder; those code checks do not narrow the OAuth grant. Google Picker and its browser API key are no longer used. Each account approves Google consent, then the app verifies the existing folder automatically. Revisions preserve prior saves, but simultaneous editing is not a transactional merge: refresh before editing shared content. Do not remove files from Drive unless you intend to remove their history or media.
